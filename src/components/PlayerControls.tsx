...
import { Wallet, TrendingUp, Leaf, Zap, CheckCircle2, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="min-h-screen bg-blue-50 text-blue-900 p-4 md:p-6 font-sans relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <motion.div whileTap={{ scale: 0.95 }} className="bg-white p-4 rounded-2xl game-border-green">
          ...
          <div className="text-2xl font-mono font-bold text-green-600">${Math.round(player.balance).toLocaleString()}</div>
        </motion.div>
        ...
        <motion.div whileTap={{ scale: 0.95 }} className="bg-white p-4 rounded-2xl game-border-blue">
          ...
          <div className="text-2xl font-mono font-bold text-blue-600">{Math.round(player.gdp_score).toLocaleString()}</div>
        </motion.div>
        ...
        <motion.div whileTap={{ scale: 0.95 }} className="bg-white p-4 rounded-2xl game-border-red">
          <div className="flex items-center gap-2 mb-1 opacity-50">
            ...
            <span className="text-[10px] font-mono font-bold uppercase">EH / SS</span>
          </div>
          <div className="text-lg font-mono font-bold text-red-600">
            {gameState.eh}%
            <span className="text-blue-500 ml-1">/ {gameState.ss}%</span>
          </div>
        </motion.div>
      </div>
